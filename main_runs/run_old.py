...
class QBucket:
    def __init__(self, num_cold_start=10, increase_rate=2, default_res=(1, 1000, 1000), max_res=(24,32000,4000)):
    ...
    def get_allocation_resource(self, res_type, last_res, res_exceeded):
        ...
        while i < num_completed_tasks:
            ...
            reg_sig = sorted_res[i].significance
            if resource <= buckets_res[ptr_bucket_resource]:
                cnt_resource += reg_sig
    ...
    def simulation_consumer(self):
        ...
        #for i in range(self.queue_length):
            ...
            if result.success:
                ...
                if res_exceeded:
                    s, e, i = self.task_id_to_task_input[task_id]
                    ...
                    self.queues.send_inputs(s, topic='simulate', method='compute_atomization_energy', keep_inputs=True, task_info={'pred': e, 'rank': i, 'resource_allocation': resource_allocation, 'tag': task_id})
                else:
                    i += 1
                    with self.qbucket_lock:
